...
InternetSpeed_performance_raw = read_csv(
  ".\\CourseWork_Ds_sabjaShrestha\\Obtained Data\\Internet Speed\\201805_fixed_pc_performance_r03.csv",
  )

InterSpeed_clean <- InternetSpeed_performance_raw %>%
  rename(
    postcode = postcode,
...
InternetSpeed_filtered <- InterSpeed_clean %>%
  filter(
...
InternetSpeed_postcode_summary <- InternetSpeed_filtered %>%
  group_by(postcode) %>%
  summarise(
...
print(InternetSpeed_postcode_summary)
